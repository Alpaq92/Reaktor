...
int  curie_snap_init(curie_a11y_action activate, curie_a11y_action focus,
                     void *user);
void curie_snap_shutdown(void);
...
int  curie_snap_update(const curie_a11y *a, unsigned focus_id);
...
int  curie_snap_get(unsigned id, curie_snap_node *out, char *buf, size_t cap);
...
unsigned curie_snap_child(unsigned parent, int last);
unsigned curie_snap_sibling(unsigned id, int back);
unsigned curie_snap_parent(unsigned id);
...
unsigned curie_snap_hit(float x, float y);

unsigned curie_snap_focus(void);
...
int curie_snap_focusable(unsigned char role, unsigned state);

/* A client's request, from the client's thread. Records it and wakes the
 * loop; the app's thread runs it in curie_snap_drain before the next frame.
...
void curie_snap_request_focus(unsigned id);
void curie_snap_request_activate(unsigned id);
void curie_snap_drain(void);

#endif /* CURIE_A11Y_SNAPSHOT_H */
