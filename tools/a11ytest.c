...
static void
dump_to(curie_a11y *a, char *out, size_t cap)
{
    FILE *f = tmpfile();
    long n;
...
static void
build(curie_a11y *a, int tab, int with_link, int checked)
{
    ...
    curie_a11y_push(a, CURIE_A11Y_TABLIST, NULL, NULL, 0, r(0, 40, 960, 34));
    for (i = 0; i < 3; i++)
        curie_a11y_add(a, CURIE_A11Y_TAB, names[i], NULL,
                       i == tab ? CURIE_A11Y_SELECTED : 0u,
                       r((float)(8 + i * 72), 40, 68, 34));
    curie_a11y_pop(a);

    curie_a11y_push(a, CURIE_A11Y_GROUP, "Buttons", NULL, 0,
                    r(0, 74, 960, 606));
    if (with_link)
        curie_a11y_add(a, CURIE_A11Y_LINK, "contact", NULL, 0,
                       r(16, 82, 60, 20));
    curie_a11y_add(a, CURIE_A11Y_BUTTON, "Continue", NULL, 0,
                   r(16, 110, 200, 38));
    curie_a11y_add(a, CURIE_A11Y_CHECKBOX, "Wrap lines", NULL,
                   checked ? CURIE_A11Y_CHECKED : 0u, r(16, 158, 160, 24));
    curie_a11y_add(a, CURIE_A11Y_TEXTBOX, "E-mail", "you@example.com",
                   CURIE_A11Y_FOCUSED, r(16, 190, 380, 38));
    curie_a11y_pop(a);
...
ids_of(curie_a11y *a, unsigned out[3])
{
    ...
    out[0] = out[1] = out[2] = 0;
    for (i = 0; i < n && k < 3; i++)
        if (t[i].role == CURIE_A11Y_BUTTON ||
            t[i].role == CURIE_A11Y_CHECKBOX ||
            t[i].role == CURIE_A11Y_TEXTBOX)
            out[k++] = t[i].id;
}
...
bench(void)
{
    ...
    for (f = 0; f < frames; f++) {
        curie_a11y_begin(&a, "Curie", r(0, 0, 960, 680));
        curie_a11y_push(&a, CURIE_A11Y_TABLIST, "Pages", NULL, 0,
                        r(0, 36, 960, 34));
        for (i = 0; i < 7; i++)
            curie_a11y_add(&a, CURIE_A11Y_TAB, labels[i % 8], NULL,
                           i == (f & 3) ? CURIE_A11Y_SELECTED : 0u,
                           r((float)(i * 72), 40, 68, 28));
        curie_a11y_pop(&a);
        curie_a11y_push(&a, CURIE_A11Y_GROUP, "Buttons", NULL, 0,
                        r(0, 70, 960, 610));
        for (i = 0; i < 24; i++)
            curie_a11y_add(&a, CURIE_A11Y_BUTTON, labels[i % 8], NULL, 0,
        ...
        for (i = 0; i < 21; i++)
            curie_a11y_add(&a, CURIE_A11Y_BUTTON, NULL, NULL, 0,
                           r((float)(20 + i * 42), 507, 34, 34));
        for (i = 0; i < 27; i++)
            curie_a11y_add(&a, CURIE_A11Y_LABEL, labels[i % 8], "v", 0,
...
{
    static curie_a11y a;      /* 600 KB of arenas; not a stack object */
    ...
    build(&a, 1, 0, 0);
    {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (c[i].kind == CURIE_A11Y_RESTATED) restated++;
            if (c[i].kind == CURIE_A11Y_RENAMED)  renamed++;
        }
    ...
    curie_a11y_begin(&a, "w", r(0, 0, 10, 10));
    for (i = 0; i < 4; i++)
        curie_a11y_add(&a, CURIE_A11Y_BUTTON, NULL, NULL, 0, r(0, 0, 1, 1));
    curie_a11y_end(&a);
    {
        int m = 0, j, dup = 0;
        const curie_a11y_node *t = curie_a11y_tree(&a, &m);
        for (i = 0; i < m; i++)
            for (j = i + 1; j < m; j++)
    ...
    a.overflow_nodes = 0;
    curie_a11y_begin(&a, "w", r(0, 0, 10, 10));
    for (i = 0; i < CURIE_A11Y_MAX_NODES + 64; i++)
        curie_a11y_add(&a, CURIE_A11Y_BUTTON, "x", NULL, 0, r(0, 0, 1, 1));
    curie_a11y_end(&a);
    curie_a11y_tree(&a, &n);
    check("overflow clamps at the arena size", n, CURIE_A11Y_MAX_NODES);
