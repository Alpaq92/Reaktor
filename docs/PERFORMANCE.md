...
| | Windows | Linux | macOS |
| --- | --- | --- | --- |
| Committed / private / footprint | **9.0 MB** | **4.3 MB** | **~30 MB** |
...
nothing. macOS settles around **0.3–0.5%**: CoreAnimation ticks the window
periodically and each tick wakes `SDL_WaitEvent` for a no-op iteration. The
cost is the wake, not a frame.
...
- **All three** — 1.00× scale, 960×680 window.
