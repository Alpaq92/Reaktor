...
| `reaktor` (GhostBSD, stripped) | 2,793,248 |
...
than 6.0.9 — so that gap is a toolchain version as much as a host. Its packaged
assets are byte-identical to Linux's at 86,001 and its `.wasm` differs by 143.
...
MIT-SHM — leaving 2.6 MB of anonymous memory that is genuinely this process's.
That one mapping is the buffer the macOS paragraph below is about, shared here
rather than copied. The column is swap-backed resident, not Linux's
private-clean-plus-dirty: `reaktor_process_memory` in `src/util.c` branches on
`_WIN32` and `__linux__` only, so on FreeBSD the app reports zero for its own
memory and these came from `ps` and `procstat -v`, as macOS came from `vmmap`.

**macOS is ~7× Linux, and the gap is outside `src/`.** `vmmap` splits the
...
The same rasteriser on all four, so Windows and Linux agreeing is the
...
or pointed at by `REAKTOR_ROOT` (the web build packages them into
`reaktor.data`), and a served web build, since a `file://` page cannot fetch
the `.wasm`.
