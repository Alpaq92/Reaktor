...
window. That coverage costs **16.38 ms** textured and blended, **6.95 ms**
untextured, **0.12 ms** as a fill.

The renderer now spots an axis-aligned quad of one color and fills it, and
skips blending when the color is opaque. A frame of this benchmark went from
**11.48 ms to 2.12 ms**, an empty window from 8.06 ms to 0.54 ms. Pixels match
to within one least significant bit, `--renderer direct3d11` to the bit. Only
measuring the present found it.
...
Every arm is held to 60 fps and sleeps out the rest of the frame, so it is
...
megabyte. Memory is peak summed private bytes across the process tree, and the
JVM runs with no heap flags.

The bars come from that outside sampler, not the apps' own frame counters,
which miss driver threads and presentation: for `direct3d11`, `bench`
extrapolated 2.5% of a core where the sampler charged 16.5%. `direct3d12` and
`vulkan` fall back to D3D11 in this SDL build.
...
Everything links statically — SDL, libcss, plutosvg, plutovg, Nuklear, Onlay,
and mojibake and kb_text_shape wherever Text is — so nothing but the C runtime
loads from outside the binary.

The Windows, Linux and macOS bundles agree within 211 bytes, so Closure and
`wasm-opt` do not vary by host (Windows and Linux on emscripten 6.0.9). The
Linux bundle is 1,221,689 bytes of `.wasm`, 86,001 of assets, 80,045 of `.js`
and 1,451 of `.html`. GhostBSD widens the spread to 380 bytes, but on
emscripten 6.0.3 — as much toolchain as host.
...
Each module off in turn, same machine and toolchain; web figures are Windows
emsdk 6.0.9. Only the showcase links them, so only the showcase is measured.
...
**The bridges are small** — off, they drop their libraries, not bytes. Pixels
and `--a11y-dump` stay byte-identical; only a screen reader notices: Windows UI
Automation finds 92 elements on the Buttons page, none without them.

**Text is mostly tables and a font.** Of its 787 KB, kb_text_shape is 113 KB of
code and 443 KB of tables, and mojibake adds its line-breaking and
...
Diagnostics is 8.0 MB with every module, 7.6 MB without Text, 8.2 MB without
Locale — Text's 0.4 MB is kb_text_shape's tables, which are not `const` and so
are committed untouched. Translations is 10.8 MB against 7.7 MB without Text:
M PLUS 1p held open to rasterize from, what kb_text_shape parses out of it, the
...
system to back, resident what is in RAM now — including shared library pages
that are not the process's to give back.

**Linux started at 123.6 MB, almost none of it ours.** SDL's X11 driver sets up
OpenGL whatever renderer you ask for, and with no GPU that means Mesa's
software OpenGL, which loads LLVM: 53 MB of libLLVM, 10 of Mesa, 30 of heap.
Compiling SDL's GL drivers out brought it to 4.3 MB; `-DREAKTOR_SDL_GL=ON` puts
them back.

**GhostBSD lands beside Linux**, within 0.5 MB resident and 0.8 MB on the other
row — the same rasterizer on the same X11 with no GPU. Of its 5.1 MB, 2.5 MB is
one SysV shared segment of 2,611,200 bytes (960×680×4): SDL's framebuffer,
handed to the X server over MIT-SHM. The other 2.6 MB is the process's own.
That column is swap-backed resident, not Linux's private clean-plus-dirty —
`reaktor_process_memory` handles only `_WIN32` and `__linux__`, so FreeBSD's
came from `ps` and `procstat -v`, macOS's from `vmmap`.

**macOS is ~7× Linux, and the gap is outside this code.** `vmmap` splits the
~30 MB into ~15 MB of Cocoa floor and ~12 MB of heap, most of it three ~2.5 MB
...
not one; that resolves about 1 MB. For finer attribution, `tools/vmwalk.c` splits another process's
private bytes into named buckets and reports what it could not account for:
...
Same rasterizer on all four, so Windows and Linux agreeing is expected. macOS
pays an extra pass: SDL's Metal renderer uploads the software bitmap to a
texture and presents that — the private copy that dominates its memory. Cost
scales with frames: sweeping the pointer along a row of buttons draws one per
hover crossing.

On GhostBSD the frame is mostly present: 6.7 ms of Login's 7.1 ms median, and
11.1 ms of Buttons' 14.8. Its X server drives VMware's SVGA II with no 3D, so a frame
is a software blit into the virtual framebuffer, then out to the host. The
other platforms' splits were not recorded; ranges are p10–p90 of 31 samples.

The accessibility tree adds **4.2 µs** to a drawn frame (82 nodes) — under the
...
of RAM** and a few hundred triangles a frame. A deployment needs the asset
files, found by walking up to the `.reaktor-root` marker, and on the web an
HTTP server, since a `file://` page cannot fetch the `.wasm` (the web build
packs the assets into its `.data` bundle).
